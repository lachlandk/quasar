...
export function parse(input) {
    ...
    output.reverse();
    if (output.length === 1) {
        expression.root = createLeafNode(output.shift());
    }
    else {
        const createNode = (token) => {
            ...
            if (output[0].type === "operator") {
                right = createNode(output.shift());
            }
            else {
                right = createLeafNode(output.shift());
            }
            if (output[0].type === "operator") {
                left = createNode(output.shift());
            }
            else {
                left = createLeafNode(output.shift());
            }
            if (token.value === "^") {
        ...
        expression.root = createNode(output.shift());
    }
    return expression;
}
